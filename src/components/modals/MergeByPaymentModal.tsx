...
interface MergeByPaymentModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSelect: (method: PaymentMethod) => void;
}

const MergeByPaymentModal: React.FC<MergeByPaymentModalProps> = ({ isOpen, onClose, onSelect }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-start md:items-center justify-center z-50 p-4 pt-16 md:pt-4" onClick={onClose}>
      <div className="relative bg-gray-800 rounded-xl shadow-2xl p-6 w-full max-w-xs" onClick={(e) => e.stopPropagation()}>
        ...
        <h2 className="text-xl font-bold text-white mb-6">Merge by Payment</h2>
        ...
        <div className="flex flex-col space-y-2">
          {Object.values(PaymentMethod).map(method => (
            ...
            >
              {`Merge ${method.toUpperCase()} Orders`}
            </button>
          ))}
        </div>
      </div>
...
export default MergeByPaymentModal;