          ...
          <div className="relative">
              ...
              {isUnitPickerOpen && (
                  <div ref={unitPickerRef} className="absolute bottom-full right-0 mb-2 bg-gray-700 rounded-lg shadow-lg p-1 z-10 max-h-64 w-32 overflow-y-auto hide-scrollbar">
                      ...
                      {(Object.values(Unit) as Unit[]).map(u => <button key={u} onClick={() => handleUnitSelect(u)} className="px-3 py-1.5 text-sm text-left rounded-md hover:bg-indigo-600 whitespace-nowrap">{u}</button>)}
                      </div>
                  </div>
              )}
          </div>
